...
keep SP_b* cvemun ent date median_lum x_t_* lgpop pob2000 bal_48* /// treatvar & controls
		p_t p_1 p_50 p_250 p_1000m e_t e_50 e_250 e_1000m   /// emp dep var  
		lg_ta_femenino lg_ta_masculino lg_voluntarios_masculino lg_voluntarios_femenino lg_voluntarios /// imss consolidated
...
foreach var of varlist p_t p_1 p_50 p_250 p_1000m { 	
	...
	*Luminosity (+ quarter of implementation)
	...
	esttab using "$directorio/Tables/reg_results/did_reg_pat.csv", se r2 ${star} b(a2)  replace keep(SP*) scalars("DepVarMean DepVarMean" "num_mun num_mun")		
...
foreach var of varlist e_t e_1 e_50 e_250 e_1000m { 	
	...
	*Luminosity (+ quarter of implementation)
	...
	esttab using "$directorio/Tables/reg_results/did_reg_emp.csv", se r2 ${star} b(a2)  replace keep(SP*) scalars("DepVarMean DepVarMean" "num_mun num_mun")	
	
	
	
******************************** IMSS ******************************************
eststo clear
foreach var of varlist lg_afiliados_imss* { 
	*B-C specification
	...
	*Luminosity (+ quarter of implementation)
	...
	esttab using "$directorio/Tables/reg_results/did_reg_imss.csv", se r2 ${star} b(a2)  replace keep(SP*) scalars("DepVarMean DepVarMean" "num_mun num_mun")	

	

******************************** IMSS ******************************************
eststo clear
foreach var of varlist lg_ta_femenino lg_ta_masculino lg_voluntarios_masculino lg_voluntarios_femenino lg_voluntarios /// imss consolidated
		 lg_trab_eventual lg_trab_perm lg_trab_campo lg_trab_urb lg_ta_sal /// asg
		 { 	
	*Luminosity (+ quarter of implementation)
	eststo : xi : xtreg `var' i.ent*date i.ent*date2 i.ent*date3 i.date lgpop x_t_* median_lum* sexo c.date#i.q_imp SP_b_F16x SP_b_F12x SP_b_F8x SP_bx SP_b_L4x SP_b_L8x SP_b_L12x SP_b_L16 [aw=pob2000] if bal_48_imss==1, fe cluster(cvemun)
		qui levelsof cvemun if e(sample)==1 
	...
	esttab using "$directorio/Tables/reg_results/did_reg_het.csv", se r2 ${star} b(a2)  replace keep(SP*) scalars("DepVarMean DepVarMean" "num_mun num_mun")	
	
	
	
******************************** IMSS ******************************************
eststo clear
foreach var of varlist lg1_masa_sal_ta* lg1_masa_sal_te lg1_masa_sal_tp lg1_masa_sal_tc lg1_masa_sal_tu /// asg
		{
	*Luminosity (+ quarter of implementation)
	eststo : xi : xtreg `var' i.ent*date i.ent*date2 i.ent*date3 i.date lgpop x_t_* median_lum* sexo c.date#i.q_imp SP_b_F16x SP_b_F12x SP_b_F8x SP_bx SP_b_L4x SP_b_L8x SP_b_L12x SP_b_L16 [aw=pob2000] if bal_48_imss==1, fe cluster(cvemun)
		qui levelsof cvemun if e(sample)==1 
	...
	esttab using "$directorio/Tables/reg_results/did_reg_sal.csv", se r2 ${star} b(a2)  replace keep(SP*) scalars("DepVarMean DepVarMean" "num_mun num_mun")			
...
foreach var of varlist lg_total_d* { 
	...
	*Luminosity
	eststo : xi : xtreg `var' i.ent*date i.ent*date2 i.ent*date3 i.date lgpop x_t_* median_lum* sexo SP_b_F16x SP_b_F12x SP_b_F8x SP_bx SP_b_L4x SP_b_L8x SP_b_L12x SP_b_L16 [aw=pob2000] if bal_48_imss==1, fe cluster(cvemun)
		qui levelsof cvemun if e(sample)==1 
		local num_mun = `r(r)'
		su `var' if e(sample)==1
		estadd scalar DepVarMean = `r(mean)'
...
*-------------------------------------------------------------------------------
	...
	*Luminosity
	eststo : xi : xtreg `var' i.ent*date i.ent*date2 i.ent*date3 i.date lgpop x_t_* median_lum* sexo SP_b_F16x SP_b_F12x SP_b_F8x SP_bx SP_b_L4x SP_b_L8x SP_b_L12x SP_b_L16 [aw=pob2000] if bal_48_d==1, fe cluster(cvemun)
		qui levelsof cvemun if e(sample)==1 
		local num_mun = `r(r)'
		su `var' if e(sample)==1
		estadd scalar DepVarMean = `r(mean)'
	...
	*Luminosity (+ quarter of implementation)
	eststo : xi : xtreg `var' i.ent*date i.ent*date2 i.ent*date3 i.date lgpop x_t_* median_lum* sexo c.date#i.q_imp SP_b_F16x SP_b_F12x SP_b_F8x SP_bx SP_b_L4x SP_b_L8x SP_b_L12x SP_b_L16 [aw=pob2000] if bal_48_d==1, fe cluster(cvemun)
		qui levelsof cvemun if e(sample)==1 
		local num_mun = `r(r)'
		su `var' if e(sample)==1
		estadd scalar DepVarMean = `r(mean)'
