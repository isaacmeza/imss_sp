...
use "$directorio\Data Original\ENE_ENEU_ENOE\ENOE\ENOEN_SDEMT120.dta", clear
keep cd_a ent con v_sel n_hog h_mud n_ren mun fac `vars'
gen int year = 2020
gen int quarter = 1
...
foreach year in 05 06 07 08 09 10 11 12 13 14 15 16 17 18 {
	foreach qr in 1 2 3 4 {
		di "`year'" "`qr'"
		qui append using "$directorio\Data Original\ENE_ENEU_ENOE\ENOE\SDEMT`qr'`year'.dta", keep(cd_a ent con v_sel n_hog h_mud n_ren mun fac `vars') 
...
foreach qr in 1 2 {
	di "`qr'"
	qui append using "$directorio\Data Original\ENE_ENEU_ENOE\ENOE\sdemt`qr'19.dta", keep(cd_a ent con v_sel n_hog h_mud n_ren mun fac `vars')
