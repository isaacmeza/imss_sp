...
foreach var of varlist lg_total_d lg_total_d_cov_sp lg_total_d_cov_isp lg_carcinoma-lg_high_blood_pressure { 
	...
	*Luminosity (+ other controls) (+ quarter of implementation)
	xi : xtreg `var' SP_b_p1-SP_b_p23 SP_b_p25-SP_b_p57 i.ent*date i.ent*date2 i.ent*date3 i.date lgpop x_t_* median_lum* sexo c.date#i.q_imp [aw=pob2000] if bal_48==1, fe cluster(cvemun)
	matrix event_bc_`var' = J(53,1,.)	
	matrix var_bc_`var' = J(53,1,.)		
	forvalues j = 5/57 {
		if `j'!=24 {
	...
	event_plot event_bc_`var'#var_bc_`var', default_look ///
		...
		title("") xlabel(-24(4)24)) stub_lag(Post#) stub_lead(Pre#) together
	...
	*de Chaisemartin, C and D'Haultfoeuille, X (2020b).  Difference-in-Differences Estimators of Intertemporal Treatment Effects.
	did_multiplegt `var' cvemun date SP_b if bal_48==1, robust_dynamic dynamic(24) placebo(24) breps(3) controls(lgpop x_t_* median_lum* sexo) weight(pob2000) cluster(cvemun)

	event_plot e(estimates)#e(variances), default_look ///
		graph_opt(xtitle("Quarters since SP adoption") ytitle("Average causal effect") ///
		title("") xlabel(-24(4)24)) stub_lag(Effect_#) stub_lead(Placebo_#) together	
