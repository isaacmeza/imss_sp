...
foreach year in 05 06 07 08 09 10 11 12 13 14 15 16 17 18 {
	foreach qr in 1 2 3 4 {
		di "`year'" "`qr'"
		qui append using "$directorio\Data Original\ENE_ENOE\ENOE\COE1T`qr'`year'.dta", keep(cd_a ent con v_sel n_hog h_mud n_ren `vars') 
...
foreach qr in 1 2 {
	di "`qr'"
	qui append using "$directorio\Data Original\ENE_ENOE\ENOE\coe1t`qr'19.dta", keep(cd_a ent con v_sel n_hog h_mud n_ren `vars')
